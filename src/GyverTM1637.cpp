...
void GyverTM1637::delayASYNC(uint32_t t) {
	uint32_t a = micros();
	while (t > 0) {
		if (*_func) _func();
...
void GyverTM1637::anamationYield(void (*func)()) {
	_func = *func;
}
#define delay(t) delayASYNC(t);
...
void GyverTM1637::runningChar(uint8_t DispData[], byte amount, int delayMs) {
	if (!_animation) {
		for (uint8_t i = 0;i < amount; i++) {
			DispData[i] = CharToByte(DispData[i]);
		}
		runningString(DispData, amount, delayMs);
	}
}

void GyverTM1637::runningString(String t, int delayMs) {
	if (!_animation) {
		byte amount = t.length();
		uint8_t DispData[amount];
		for (uint8_t i = 0;i < amount; i++) {
			DispData[i] = CharToByte(t.charAt(i));
		}
		runningString(DispData, amount, delayMs);
	}
}
...
uint8_t CharToByte(char t) {
	switch (t) {
		...
		case 'B': return 0x7f; break;
		case 'C': return 0x39; break;
		case 'D': return 0x3f; break;
		...
		case 'a': return 0x5f; break;
		case 'b': return 0x7c; break;
		case 'c': return 0x58; break;
		case 'd': return 0x5e; break;
		case 'e': return 0x7b; break;
		case 'f': return 0x71; break;
		case 'h': return 0x74; break;
		case 'i': return 0x10; break;
		case 'j': return 0x0e; break;
		case 'l': return 0x06; break;
		...
		case 176: return 0x63; break; //degree, use char(176)
